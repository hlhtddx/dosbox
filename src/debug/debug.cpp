	...
	while (*hex) {
		if ((*hex >= '0') && (*hex <= '9')) value = (value << 4) + *hex - '0';
			...
			if (*hex == '+') { hex++; return regval + value + GetHexValue(hex, hex); } else
				if (*hex == '-') { hex++; return regval + value - GetHexValue(hex, hex); } else break; // No valid char
		}
		hex++;
	};
	return regval + value;
...
#if C_HEAVY_DEBUG
void LogFillFlags(bool cf) {
	if (!cpuLog)
		return;

	if (cf) {
		cpuLogFile << hex << "FillFlags: " << lflags.type << "flags:" << setw(8) << reg_flags << endl;

	} else {
		cpuLogFile << hex << "FillFlagsNoCFOF: " << lflags.type << "flags:" << setw(8) << reg_flags << endl;
	}
}
