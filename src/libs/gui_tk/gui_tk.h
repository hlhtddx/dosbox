...
namespace GUI {
	...
	protected:
		...
		struct ltuint {
			bool operator()(Ticks i1, Ticks i2) const {
				return (i1 < i2);
			}
		};
	...
	public:
		...
		virtual bool keyDown(const Key &key) {
			...
			else if (key.special == Key::Enter) { execute(); return true; } else if (key.special == Key::Escape) { setVisible(false); return true; } else return true;
