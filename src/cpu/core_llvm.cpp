...
#define CHECK_ALL_INSTRUCTIONS 1
#if CHECK_ALL_INSTRUCTIONS
bool inst_used[0x300] = { 0 };
void dump_inst_used() {
...
restart_opcode:
		Bit8u insb = Fetchb();
#if CHECK_ALL_INSTRUCTIONS
		inst_used[(core.opcode_index + insb) & 0x2ff ] = true;
#endif
		switch (core.opcode_index + insb) {
...
#if CPU_TRAP_CHECK
			if (GETFLAG(TF)) {
				cpudecoder = CPU_Core_LLVM_Trap_Run;
				return CBRET_NONE;
			}
#endif
			continue;
