...
CASE_D(0x87)												/* XCHG Ed,Gd */
{
	...
	if (rm >= 0xc0) { GetEArd; *rmrd = *eard; *eard = oldrmrd; } else { GetEAa; *rmrd = LoadMd(eaa); SaveMd(eaa, oldrmrd); }
	break;
}
CASE_D(0x89)												/* MOV Ed,Gd */
{
	GetRMrd;
	if (rm >= 0xc0) { GetEArd; *eard = *rmrd; } else { GetEAa; SaveMd(eaa, *rmrd); }
	break;
}
CASE_D(0x8b)												/* MOV Gd,Ed */
{
	GetRMrd;
	if (rm >= 0xc0) { GetEArd; *rmrd = *eard; } else { GetEAa; *rmrd = LoadMd(eaa); }
	break;
}
CASE_D(0x8c)												/* Mov Ew,Sw */
{
	...
	if (rm >= 0xc0) { GetEArd; *eard = val; } else { GetEAa; SaveMw(eaa, val); }
	break;
}
CASE_D(0x8d)												/* LEA Gd */
{
...
CASE_D(0x8f)												/* POP Ed */
{
	Bit32u val = Pop_32();
	GetRM;
	if (rm >= 0xc0) { GetEArd; *eard = val; } else { GetEAa; SaveMd(eaa, val); }
	break;
}
...
{
	GetRM;
	if (rm >= 0xc0) { GetEArd; *eard = Fetchd(); } else { GetEAa; SaveMd(eaa, Fetchd()); }
	break;
}
...
CASE_D(0xf7)												/* GRP3 Ed(,Id) */
{
	...
	{
		if (rm >= 0xc0) { GetEArd; TESTD(*eard, Fetchd(), LoadRd, SaveRd); } else { GetEAa; TESTD(eaa, Fetchd(), LoadMd, SaveMd); }
		break;
	}
	case 0x02:											/* NOT Ed */
	{
		if (rm >= 0xc0) { GetEArd; *eard = ~*eard; } else { GetEAa; SaveMd(eaa, ~LoadMd(eaa)); }
		break;
	}
	case 0x03:											/* NEG Ed */
	{
...
CASE_D(0xff)												/* GRP 5 Ed */
{
	...
	case 0x02:											/* CALL NEAR Ed */
		if (rm >= 0xc0) { GetEArd; reg_eip = *eard; } else { GetEAa; reg_eip = LoadMd(eaa); }
		Push_32(GETIP);
		continue;
	...
	break;
	case 0x06:											/* Push Ed */
		if (rm >= 0xc0) { GetEArd; Push_32(*eard); } else { GetEAa; Push_32(LoadMd(eaa)); }
		break;
	default:
