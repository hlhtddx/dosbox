...
class InstructionState(State):
    ...
    def __generate_code(self, context, output, operands, mode):
        ...
        if operand_width:
            if operands.address['dst'] and cnv_type['dst']:
                self.__generate_destination_code(context, output, operands.address['dst'][0])
            if operands.address['src'] and cnv_type['src']:
                self.__generate_source_code(context, output, operands.address['src'][0])
    ...
    def __generate_destination_code(self, context, output, address_type):
        address = self.__generate_address_code(address_type)
        context.write_indent_line(output, f'auto destination = {address};')

    def __generate_source_code(self, context, output, address_type):
        address = self.__generate_address_code(address_type)
        context.write_indent_line(output, f'auto source = {address};')

    @staticmethod
    def __generate_code_rm_mod(reg_type):
        return f'GetRmMod<data_type>(rmmod)'

    @staticmethod
    def __generate_code_rm_rm(reg_type):
        return f'GetRmRm<data_type>(rmmod)'

    @staticmethod
    def __generate_code_rm_reg(reg_type):
        return f'GetRmReg<data_type>(rmmod)'

    @staticmethod
    def __generate_code_rm_mem(reg_type):
        return f'GetRmMem<data_type>(rmmod)'

    @staticmethod
    def __generate_code_reg_gen(reg_num):
        return f'GetGeneralRegister<data_type>({reg_num})'

    @staticmethod
    def __generate_code_immediate():
        return f'GetImmediate<data_type>()'

    def __generate_address_code(self, address_type):
        if address_type[0] == 'rm':

            if address_type[1] == 'rm':
                return self.__generate_code_rm_rm(address_type[2])
            elif address_type[1] == 'mod':
                return self.__generate_code_rm_mod(address_type[2])
            elif address_type[1] == 'reg':
                return self.__generate_code_rm_reg(address_type[2])
            elif address_type[1] == 'mem':
                return self.__generate_code_rm_mem(address_type[2])
            else:
                raise X86Error('Unknown operand format type')

        elif address_type[0] == 'reg':
            if address_type[1] == 'gen':
                return self.__generate_code_reg_gen(address_type[2])
            elif address_type[1] == 'seg':
                return self.__generate_code_reg_gen(address_type[2])
            else:
            ...
            if address_type[1] == 'gen':
                return self.__generate_code_mem_gen(address_type[2])
            elif address_type[1] == 'imm':
                return self.__generate_code_mem_imm(address_type[2])
            else:
                raise X86Error('Unknown operand register type')
