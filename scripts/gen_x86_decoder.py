...
class StateTransition:
    def __init__(self, src_state, dst_state, trigger, handler):
...
class InstructionState(State):
    ...
    @property
    def handler_name(self):
        s = f'handler_{self.instruction.mnemonic}<addr_{address_size_specifiers[self.address_size]}'

        if self.dst_op_fmts:
            s += f',t{self.dst_op_fmts.address_string},t_{self.dst_op_fmts.data_type}{self.dst_op_fmts.data_size}'
        else:
            s += ',void,void'

        if self.src_op_fmts:
            s += f',t{self.src_op_fmts.address_string},t_{self.src_op_fmts.data_type}{self.src_op_fmts.data_size}'
        else:
            s += ',void,void'
    ...
    @property
    def handler_implement(self):
        return f'handler_{self.instruction.mnemonic}'
    ...
    def generate_declaration(self, context, output):
        ...
        output.write(f'{context.indention}template<typename addr, typename dst_addr, typename dst_type'
                     f', typename src_addr, typename src_type>\n')
...
class StateTransitionGraph:
    ...
    def write(self, outfile):
        self.write_imports(outfile)
        self.write_template(outfile)
        self.write_declarations(outfile)
        self.write_definitions(outfile)
