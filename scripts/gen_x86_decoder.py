...
class InstrHandler:
    def __init__(self, instr, oper_size, addr_size):
        ...
        self.op_fmts = instr.operand_fmts
        self.oper_size = oper_size
    ...
    def unique_name(self):
        name = '%s_%s' % (self.instruction.mnemonic, address_width[self.addr_size])
        for fmt in self.op_fmts:
            name += '_%s' % fmt.unique_sized_name(self.oper_size)
        return name

    def generate_function(self, output):
        output.write('template<%s, %s> OpState %s(Bit8u last, PhysPt &cseip) {' % (operand_width[self.oper_size], address_width[self.addr_size], self.unique_name()));
        output.write('}' % ());
...
def InsertInstruction(ins):
    ...
    for opcode in op_path:
        ...
        if isinstance(item, Instruction) or item == None:
            ...
            state_map[(operand_width[0], address_width[0]), new_node.base] = new_node
            item = new_node
...
for oper_size in operand_width:
    for addr_size in address_width:
...
def WriteTransitStatesDefinition(output):
    ...
    for key in option_prefixes:
        output.write('\t%s::table_%s_%s%s,\n' % (class_name, operand_width[0], address_width[0], OpcodeToPath(keys[0][1])))
    output.write('};\n')
